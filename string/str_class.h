...
template <typename T>
	...
	*/
	...
	virtual	hash_t					hashCode()	const;
		...
		template <class T0>
			bool					operator>=(const StringExpression<T0>&expression)	const;
	virtual	serial_size_t			serialSize(bool export_size) const;	//!< Queries the serial size of this object @param export_size True if the result should include additional space required for any size variable such as a string or array length @return Serial size (in bytes) of this object
	virtual	bool					serialize(IWriteStream&out_stream, bool export_size) const;	//!< Writes the serialized content of the local object to the specified stream. The size of the written data must match the result of serialSize() as long as the object remains unchanged. @param out_stream Stream to write to @param export_size True if the object is expected to export any dynamic size such as string or array length @return true if the object could be fully written to the out stream, false otherwise
	virtual	bool					deserialize(IReadStream&in_stream, serial_size_t fixed_size);		//!< Reads the content of the local object from the specified stream @param in_stream Stream to read from @param fixed_size 0 if any dynamic object size should be read from the stream or the size in bytes that the local object should adopt to otherwise. If this parameter is non-0 then the object data was serialized with @b export_size set to false @return true if the object data could be fully read from the out stream, false otherwise
