		...
		*/
		...
		namespace GLShader
		{
			...
			static	Buffer<LightShadowAttachment,0,Swap>		shadow_attachments;
					...
					*/
					...
					public:
							...
							//RootBlock			*includes;		//!< Non-NULL if this line includes another block template following the above line segments
			...
			/*
										...
										{\n\
											vec3 ldir = normalize(gl_LightSource[<i>].position.xyz-position.xyz);\n\
											gl_FrontColor += clamp(dot(normal,ldir),0.0,1.0)*gl_FrontLightProduct[<i>].diffuse;\n\
											gl_FrontColor += clamp(dot(reflected,ldir),0.0,1.0)*gl_FrontLightProduct[<i>].specular;\n\
