	...
	namespace Fractal
	{
		...
		*/
		struct TSurfaceSegment:public TBaseSurfaceSegment
		{
			...
			Container::Buffer0<TCrater>	crater_field;		//!< Applied crater field
			Array<float>				vertices;			//!< Vertex buffer object containing renderable vertex floats. Each vertex provides 9 floats, the first 3 for the sector relative position, 1 for the height, 3 for the normal, the final 2 for texture coordinates
			Image						texture;			//!< Color/normal texture assigned to this surface
			bool						wantMipmap=false,wantCompressedTexture=false;
