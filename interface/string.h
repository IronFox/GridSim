...
#include "common.h"
/**
	@brief Generic IExceptionString interface
...
template <typename T>
	...
	public:
			...
			*/
	virtual	const T*		resolveContent()	const=0;
	};
	...
	public:
		...
		*/
		...
		/**
			...
			@return Converted string pointing to the local character storage or directly to the result of string.resolveContent() if conversion is not required. The returned string is never NULL and always zero-terminated. Deletion of the result is not allowed
		*/
		template <typename FromType>
