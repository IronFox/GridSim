...
namespace DeltaWorks
{
	...
	namespace System
	{
		...
		public:
			...
			bool				AwaitCompletion(DWORD maxWaitMilliseconds=0xFFFFFFFF);		//!< Waits until thread has returned. Do not call from the thread itself
		...
		*/
		template <class Entry>
			class BlockingQueue:private Container::Queue<Entry>
			{
			private:
					Semaphore					semaphore;
					Mutex						mutex;
		...
		*/
		template <class Entry>
			class BlockingPriorityQueue:private Container::PriorityQueue<Entry,int>
			{
			private:
					Semaphore					semaphore;
					Mutex						mutex;
