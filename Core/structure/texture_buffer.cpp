...
namespace DeltaWorks
{
	...
	{
		Riff::File  riff;
		if (comment.GetLength() > TypeInfo<Riff::RIFF_SIZE>::max)
		...
		riff.AppendBlock("CONT",comment.c_str(),Riff::RIFF_SIZE(comment.GetLength()));
		Super::VisitAllKeys([&riff](const CGS::TextureA&tex)
		{
			riff.AppendBlock("ID  ",&tex.name,sizeof(tex.name));
			for (BYTE k = 0; k < tex.face_field.GetLength(); k++)
			{
	...
	String TextureBuffer::GetState()
	{
		...
		{
			rs+=" ("+String(cnt)+"): \""+name2str(tex.name)+"\" "+String(tex.face_field.GetLength())+" face(s)\n";
			cnt++;
		});
		return rs;
	}
