...
namespace DeltaWorks
{
	...
	static  TKeyName    K_ID[]=
						{
							...
							KN(F11),KN(F12),KN(F13),KN(F14),KN(F15),KN(F16),KN(F17),KN(F18),KN(F19),KN(F20),
	...
	bool isFloat(const char*string)
	{
		...
		return c!=string && *(c-1) != globalDecimalSeparator;
	}
	...
	template <typename T>
		static T extractFloatType(const char*string, size_t length)
		{
			...
			if (c < end && *c==globalDecimalSeparator)
			{
				int sub_level=1;
				c++;
				while (c < end && *c >= '0' && *c <= '9')
