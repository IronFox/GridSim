...
template <typename T>
bool	File::InsertBlock(TID id, const ArrayRef<T>&data)
{
	return InsertBlock(id,data.pointer(),(RIFF_SIZE)data.GetContentSize());
}

template <typename T>
bool	File::AppendBlock(TID id, const ArrayRef<T>&data)
{
...
template<typename T>
Chunk*		Chunk::InsertBlock(TID id, const ArrayRef<T>&data)
{
	return InsertBlock(id,data.pointer(),data.GetContentSize());
}

template<typename T>
Chunk*		Chunk::AppendBlock(TID id, const ArrayRef<T>&data)
{
