...
namespace Magic //! Multi format image access namespace
{
		...
		*/
		...
		/**
			...
			\return true on success
		*/
        void        	loadFromFile(Image&target, const String&filename, const char*extension_override);
        void        	saveToFile(const Image&image, const String&filename);	//!< Attempts to save an image to the specified file \param filename File to save to. The function fails if \b filename does not contain any supported extension \param image Image container to read image data from \param error_out Pointer to write an error description to or NULL if no error description is required. The contents are left unchanged if the function returns true \return true on success
        void        	saveToFile(const Image&image, const String&filename, const char*extension_override);//!< Extension specific version of the above. If the specified extension is NULL then this function behaves identical to the above, \param filename File to save to \param extension_override Pointer to a zero terminated string describing the extension to use instead of any extension found in the filename. May be NULL. \param image Image container to read image data from \param error_out Pointer to write an error description to or NULL if no error description is required. The contents are left unchanged if the function returns true \return true on success
}
